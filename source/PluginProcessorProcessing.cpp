...
void RaveAP::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  // fade parameters (taken from raw audition)
  ...
  // std::cout << "current refresh rate : " << currentRefreshRate  << "; buffer
  // length : " << _inBuffer[0].len() << std::endl;
  ...
  // std::cout << "buffer out length : " << _outBuffer[0].len() << std::endl;
...
#endif
  // Compute output RMS for the GUI
  // _outputAmplitudeL = buffer.getRMSLevel(0, 0, nSamples);
  // _outputAmplitudeR = buffer.getRMSLevel(1, 0, nSamples);
}
